...
function isAllowed(term, relPath, frontmatter, context) {
  ...
  for (const e of allowed) {
    ...
    // Evidence beginning with PENDING (including annotated forms like
    // 'PENDING - …') documents a reopen condition — treat as deny.
    if (String(e.evidence || '').trim().toUpperCase().startsWith('PENDING')) continue;
    const where = e.where || '';
